# DEBUGGING AND PROFILING THE SYSTEM_INFO SCRIPT
## Facilitating debugging and profiling logs, using file descriptors, traps, and other bash built-ins
...
Likewise, a means to "profile" the script has been added.
...
how long the line in question took, to complete.

The profiling log is created using file descript #4, and the
bash shell's "PS4" environment variable.  PS4 is the shell prompt
used to display any lines of code executed when "set -x" is in
effect.
...
As described in the comments, a corresponding "set +x" is used at
both the end of the script, and in the fnABORT function, to ensure
fd4 gets closed when the script ends.
```
#---------------
