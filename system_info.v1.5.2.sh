...
#   sysbench, sysstat, systemd-coredump, watchdog, wiringpi, x11-xserver-utils
#
# NOTE:
...
then
  ...
  then
    ...
    SUPPLEMENTAL=$(dpkg -l 2>/dev/null | awk '{ print $2 }' | grep -i "^cups-client$\|^dc$\|^ethtool$\|^lvm2$\|^m4$\|^mdadm$\|^nfs-kernel-server$\|^nmap$\|^perl-base$\|^python3-gpiozero$\|^quota$\|^rng-tools$\|^rpcbind$\|^rtl-sdr$\|^samba$\|^sysbench$\|^sysstat$\|^systemd-coredump$\|^watchdog$\|^wiringpi$\|^x11-xserver-utils$")
    SUP_HIT=0
    SUP_MAX=0
    for PACKAGE in cups-client dc ethtool lvm2 m4 mdadm nfs-kernel-server nmap perl-base python3-gpiozero quota rng-tools rpcbind rtl-sdr samba sysbench sysstat systemd-coredump watchdog wiringpi x11-xserver-utils
    do
