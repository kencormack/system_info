...
# system_info.sh
# Ken Cormack
# (minor updates by William Stearns, william.l.stearns@gmail.com)
# March, 2020
#
...
# made to make things easy to follow for new admins to learn from, and the
# coding style is meant for easy readability.
#
...
fnREQUIRE() {
  ...
  while [ -n "${1}" ]
  do
    if ! type -path "${1}" >/dev/null 2>/dev/null
    then
...
fnREQUIRE amixer aplay apt arecord awk bc bluetoothctl cat cut date dc df dmesg dpkg free gpio grep hwclock i2cdetect ifconfig ip iwconfig iwlist ls lsblk lscpu lshw lsipc lsmod lsusb modinfo route ps sed sort strings stty swapon systemctl tvservice uptime vcgencmd || echo "Missing utility, please install" >&2
...
then
  INSTALLED=$(dpkg -l | awk '{ print $2 }' | grep -i "^lshw$\|^usbutils$\|^util-linux$\|^alsa-utils$\|^bluez$\|^wireless-tools$\|^bc$\|^dc$\|^i2c-tools$\|^libraspberrypi-bin$\|^wiringpi$")
...
then
  ...
  then
    ...
    echo "Missing utility lpstat, skipping printer status display" >&2
  fi
fi
...
then
  ...
  then
    ...
    | sed 's/psk=.*/psk=PASSWORD_NOT_SHOWN/' \
    | sed 's/wep_key0=.*/psk=PASSWORD_NOT_SHOWN/' \
    | sed 's/password=.*/psk=PASSWORD_NOT_SHOWN/' \
    | sed 's/passwd=.*/psk=PASSWORD_NOT_SHOWN/'
    echo
  else
...
#---------------
if type -path nmap >/dev/null 2>&1 && type -path ifconfig >/dev/null 2>&1 && type -path awk >/dev/null 2>&1 && type -path grep >/dev/null 2>&1
then
  ifconfig | grep inet | awk '{ print $2 }' | while read MY_IP
  do
    fnBANNER " SCANNING FOR SERVICES LISTENING ON: ${MY_IP}"
    nmap -sV -T4 -p 1-65535 --version-light ${MY_IP} | grep "^PORT\|^[1-9][0-9]"
    echo
  done
else
  echo "Missing one of nmap, ifconfig, awk, or grep.  Skipping port scan display" >&2
fi
...
#   lsmod >/tmp/.lsmod.${PPID}
#   HEAD="$(head -1 /tmp/.lsmod.${PPID})"
#   echo "${HEAD}"
#   cat /tmp/.lsmod.${PPID} | sort | grep -v "Used by"
#   rm /tmp/.lsmod.${PPID}
#   echo
# else
...
# This next module will likely never see the light of day.
