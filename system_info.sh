...
#
# BONUS...
# If the script also finds the "cups-client" and "nmap" packages installed, it
# will report on any configured printers via lpstat, and perform port scans of
# any configured network interfaces via nmap, to show any service daemons that
# may be listening on them.  These two packages are not requirements for the
...
then
  ...
  then
    ...
    echo "Missing utility grep, unable to verify kernel ring buffer status" >&2
  fi
else
  echo "Missing utility dmesg, unable to verify kernel ring buffer status" >&2
fi
...
then
  REQUIRED=$(dpkg -l 2>/dev/null | awk '{ print $2 }' | grep -i "^lshw$\|^usbutils$\|^util-linux$\|^alsa-utils$\|^bluez$\|^wireless-tools$\|^bc$\|^dc$\|^i2c-tools$\|^libraspberrypi-bin$\|^wiringpi$")
  OPTIONAL=$(dpkg -l 2>/dev/null | awk '{ print $2 }' | grep -i "^cups-client$\|^nmap$\|^rpcbind$\|^nfs-kernel-server$\|^samba$")

  # Check for the required packages
  #################################
  for PACKAGE in alsa-utils bc bluez dc i2c-tools libraspberrypi-bin lshw usbutils util-linux wireless-tools wiringpi
  do
    ...
    then
      ...
      fnBANNER "Required package \"${PACKAGE}\" is not installed."
      echo "Install with:"
  ...
  then
    ...
    # Now, check for the optional packages.
    # If installed, great.  If not installed, don't trouble the user to add them.
    ######################################
    # For optional PRINTER STATUS (CUPS)
    if [ -n "$(echo "${OPTIONAL}" | grep -i "^cups-client$")" ]
    then
      if type -path lpstat >/dev/null 2>&1
      then
        echo "  (Also found optional cups-client ........ we might use that, too.)"
      fi
    fi
    # For optional SCANNING FOR SERVICES
    if [ -n "$(echo "${OPTIONAL}" | grep -i "^nmap$")" ]
    then
      if type -path nmap >/dev/null 2>&1
      then
        echo "  (Also found optional nmap ............... we might use that, too.)"
      fi
    fi
    # For optional PORTMAPPER - RPCINFO
    if [ -n "$(echo "${OPTIONAL}" | grep -i "^rpcbind$")" ]
    then
      PS_RPC=$(ps -ef | grep [r]pcbind)
      if [ -n "${PS_RPC}" ]
      then
        echo "  (Also found optional rpcbind ............ we might use that, too.)"
      fi
    fi
    # For optional EXPORTED NFS DIRS
    if [ -n "$(echo "${OPTIONAL}" | grep -i "^nfs-kernel-server$")" ]
    then
      PS_NFS=$(ps -ef | grep [n]fsd)
      if [ -n "${PS_NFS}" ]
      then
        echo "  (Also found optional nfs-kernel-server .. we might use that, too.)"
      fi
    fi
    # For optional SMBSTATUS
    if [ -n "$(echo "${OPTIONAL}" | grep -i "^samba$")" ]
    then
      PS_SMB=$(ps -ef | grep [s]mbd)
      if [ -n "${PS_SMB}" ]
      then
        echo "  (Also found optional samba .............. we might use that, too.)"
      fi
    fi
...
then
  ...
  # models only.  Any other model will not run this test.
  #
  fnBANNER " OTP BOOT-FROM-USB STATUS"
  # First, if a Model 2B, make sure it's a v1.2 unit
  MY_OTP=${MY_MODEL}
...
then
  ...
  echo "Missing utility lshw, skipping input generic listing" >&2
fi
...
then
  ...
  echo "Missing utility lshw, skipping input hardware listing" >&2
fi
...
then
  ...
  echo "Missing utility cat, skipping meminfo display" >&2
fi
...
then
  ...
  echo "Missing utility gpio (part of wiringpi), skipping gpio pin status display" >&2
fi
...
#---------------
# Another optional test.  If they have the portmapper and NFS running,
...
then
  ...
  then
    ...
    echo "Missing df, skipping nfs exports display" >&2
  fi
fi

#---------------
# Another optional test.  If they have the smb running,
...
then
  ...
  then
    ...
    echo "No CIFS/Windows shares mounted"
  fi
  echo
else
  echo "Missing df, skipping nfs exports display" >&2
