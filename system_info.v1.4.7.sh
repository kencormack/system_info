...
then
  ...
  then
    ...
    SUPPLEMENTAL=$(dpkg -l 2>/dev/null | awk '{ print $2 }' | grep -i "^cups-client$\|^dc$\|^ethtool$\|^nfs-kernel-server$\|^nmap$\|^python3-gpiozero$\|^rng-tools$\|^rpcbind$\|^rtl-sdr$\|^samba$\|^sysstat$\|^systemd-coredump\|^watchdog$\|^wiringpi$\|^x11-xserver-utils$")
    SUP_HIT=0
    SUP_MAX=0
    for PACKAGE in cups-client dc ethtool nfs-kernel-server nmap python3-gpiozero rng-tools rpcbind rtl-sdr samba sysstat systemd-coredump watchdog wiringpi x11-xserver-utils
    do
...
#HITS=$(find /etc/security/limits* -type f -exec grep core {} \; -print | grep -v ^#)
...
#HITS=$(find /etc/systemd/system.conf -type f -exec grep DefaultLimitCORE {} \; -print | grep -v ^#)
