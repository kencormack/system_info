...
This script attempts to perform a fairly complete audit of the current configuration of a Raspberry Pi.  The target hardware is any variant or model of Pi, up to and including the Pi 4B in all it's available memory configurations (1/2/4GB).  Supported OS versions include Raspbian Stretch (including the newly renamed "Raspberry Pi OS" in 32-bit versions), and Buster.  No attempts will be made to back-port this to Jessie or older.
```
