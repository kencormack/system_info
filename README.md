...
Each time a report is run, a report file is created in the home directory of the userid running the script.  If you launch the script as user "pi", the report will end up in /home/pi, pi's homedir.  If you launch the script with "sudo", sudo makes you "root", thus the report will end up in /root (root's homedir).  The filename of the report contains the hostname, the name of the script, and the date and time the report was run.
