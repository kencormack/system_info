...
#   lshw, usbutils, util-linux, v4l-utils, wireless-tools
#
...
then
  ...
  REQUIRED=$(dpkg -l 2>/dev/null | awk '{ print $2 }' | grep -i "^alsa-utils$\|^bluez$\|^coreutils$\|^i2c-tools$\|^iproute2$\|^libraspberrypi-bin$\|^lshw$\|^util-linux$\|^usbutils$\|^v4l-utils$\|^wireless-tools$")
  REQ_HIT=0
  REQ_MAX=0
  for PACKAGE in alsa-utils bluez coreutils i2c-tools iproute2 libraspberrypi-bin lshw usbutils util-linux v4l-utils wireless-tools
  do
  ...
  then
    ...
    then
      ...
      then
        ...
        echo "Installed version of wiringpi does not support this Pi."
      fi
    fi
    let SUP_MAX++
...
then
  ...
  if [ "${MY_MODEL}" = "4B" ] && [ "$(echo "${REVISION}" | cut -c2-)" = "03111" ]
  then
    echo "This 4B contains a USB-C power design flaw."
    echo "\"Smart\" USB-C cables will not power this Pi."
    echo
  fi
  if [ "${MY_MODEL}" = "4B" ] && [ "$(echo "${REVISION}" | cut -c2-)" = "03112" ]
  then
    echo "This 4B v1.2 board corrects an earlier USB-C power design flaw."
    echo
  fi
fi
...
# See - http://wiringpi.com/wiringpi-updated-to-2-52-for-the-raspberry-pi-4b/
# The command will choke if not a sufficient version, so we'll run it here
# silently, and check the return code.
...
then
  ...
  echo "Missing utility gpio (part of wiringpi), skipping gpio display" >&2
fi
