...
then
  ...
  REQUIRED=$(dpkg -l 2>/dev/null | awk '{ print $2 }' | grep -i "^alsa-utils$\|^bluez$\|^coreutils$\|^i2c-tools$\|^iproute2$\|^libraspberrypi-bin$\|^lshw$\|^rpi-eeprom$\|^util-linux$\|^usbutils$\|^v4l-utils$\|^wireless-tools$")
  REQ_HIT=0
  REQ_MAX=0
  for PACKAGE in alsa-utils bluez coreutils i2c-tools iproute2 libraspberrypi-bin lshw rpi-eeprom usbutils util-linux v4l-utils wireless-tools
  do
...
then
  ...
  echo "  Newer bootloaders have this set to 1."
  echo
