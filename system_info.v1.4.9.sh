...
then
  ...
  then
    ...
    SUPPLEMENTAL=$(dpkg -l 2>/dev/null | awk '{ print $2 }' | grep -i "^cups-client$\|^dc$\|^ethtool$\|^lvm2\|^mdadm\|^nfs-kernel-server$\|^nmap$\|^python3-gpiozero$\|^quota$\|^rng-tools$\|^rpcbind$\|^rtl-sdr$\|^samba$\|^sysstat$\|^systemd-coredump\|^watchdog$\|^wiringpi$\|^x11-xserver-utils$")
    SUP_HIT=0
    SUP_MAX=0
    for PACKAGE in cups-client dc ethtool lvm2 mdadm nfs-kernel-server nmap python3-gpiozero quota rng-tools rpcbind rtl-sdr samba sysstat systemd-coredump watchdog wiringpi x11-xserver-utils
    do
...
then
  ...
  do
    fnBANNER " RAID ARRAY DEVICE /dev/${MD} (***)" | tr [a-z] [A-Z]
    ...
    do
      fnBANNER " RAID ARRAY DEVICE /dev/${MD} - COMPONENT ${DEV} (***)" | tr [a-z] [A-Z]
