...
then
  ...
  REQUIRED=$(dpkg -l 2>/dev/null | awk '{ print $2 }' | grep -i "^alsa-utils$\|^bluez$\|^coreutils$\|^i2c-tools$\|^iproute2$\|^libraspberrypi-bin$\|^lshw$\|^util-linux$\|^usbutils$\|^wireless-tools$\|^wiringpi$")
  REQ_HIT=0
  REQ_MAX=0
  for PACKAGE in alsa-utils bluez coreutils i2c-tools iproute2 libraspberrypi-bin lshw usbutils util-linux wireless-tools wiringpi
  do
  ...
  then
    ...
    SUPPLEMENTAL=$(dpkg -l 2>/dev/null | awk '{ print $2 }' | grep -i "^cups-client$\|^dc$\|^nmap$\|^rpcbind$\|^nfs-kernel-server$\|^samba$\|^sysstat$\|^watchdog$\|^python3-gpiozero$")
    SUP_HIT=0
    SUP_MAX=0
...
# The broadcom driver rquires two modules be running.
# The "fake" OpenGL driver shows "firmwarekms" in dmesg.
# The "full" OpenGL driver does neither.
...
#---------------
fnBANNER " CODECS"
# These two codecs require a license key, purchased seperately
...
# The rest of the codecs do not require seperate purchase
...
#---------------
# Packages can be placed "on hold" to prevent upgrading the package
