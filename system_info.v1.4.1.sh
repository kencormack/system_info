...
then
  ...
  REQUIRED=$(dpkg -l 2>/dev/null | awk '{ print $2 }' | grep -i "^alsa-utils$\|^bc$\|^bluez$\|^coreutils$\|^i2c-tools$\|^iproute2$\|^libraspberrypi-bin$\|^lshw$\|^net-tools$\|^rpi-eeprom$\|^util-linux$\|^usbutils$\|^v4l-utils$\|^wireless-tools$")
  REQ_HIT=0
  REQ_MAX=0
  for PACKAGE in alsa-utils bc bluez coreutils i2c-tools iproute2 libraspberrypi-bin lshw net-tools rpi-eeprom usbutils util-linux v4l-utils wireless-tools
  do
  ...
  then
    ...
    SUP_HIT=0
    SUP_MAX=0
    for PACKAGE in cups-client dc ethtool nfs-kernel-server nmap python3-gpiozero rpcbind rtl-sdr samba sysstat watchdog wiringpi
    do
...
then
  ...
  BOOT_UART=$(vcgencmd bootloader_config | grep BOOT_UART)
  if [ -n "${BOOT_UART}" ]
  then
    echo "${BOOT_UART}"
  ...
  WAKE_ON_GPIO=$(vcgencmd bootloader_config | grep WAKE_ON_GPIO)
  if [ -n "${WAKE_ON_GPIO}" ]
  then
    echo "${WAKE_ON_GPIO}"
  ...
  POWER_OFF_ON_HALT=$(vcgencmd bootloader_config | grep POWER_OFF_ON_HALT)
  if [ -n "${POWER_OFF_ON_HALT}" ]
  then
    echo "${POWER_OFF_ON_HALT}"
  ...
  DHCP_TIMEOUT=$(vcgencmd bootloader_config | grep DHCP_TIMEOUT)
  if [ -n "${DHCP_TIMEOUT}" ]
  then
    echo "${DHCP_TIMEOUT}"
  ...
  DHCP_REQ_TIMEOUT=$(vcgencmd bootloader_config | grep DHCP_REQ_TIMEOUT)
  if [ -n "${DHCP_REQ_TIMEOUT}" ]
  then
    echo "${DHCP_REQ_TIMEOUT}"
  ...
  TFTP_FILE_TIMEOUT=$(vcgencmd bootloader_config | grep TFTP_FILE_TIMEOUT)
  if [ -n "${TFTP_FILE_TIMEOUT}" ]
  then
    echo "${TFTP_FILE_TIMEOUT}"
  ...
  TFTP_IP=$(vcgencmd bootloader_config | grep TFTP_IP)
  if [ -n "${TFPT_IP}" ]
  then
    echo "${TFTP_IP}"
  ...
  TFTP_PREFIX=$(vcgencmd bootloader_config | grep TFTP_PREFIX)
  if [ "${TFTP_PREFIX}" ]
  then
    echo "${TFTP_PREFIX}"
  ...
  BOOT_ORDER=$(vcgencmd bootloader_config | grep BOOT_ORDER)
  if [ -n "${BOOT_ORDER}" ]
  then
    echo "${BOOT_ORDER}"
  ...
  SD_BOOT_MAX_RETRIES=$(vcgencmd bootloader_config | grep SD_BOOT_MAX_RETRIES)
  if [ -n "${SD_BOOT_MAX_RETRIES}" ]
  then
    echo "${SD_BOOT_MAX_RETRIES}"
  ...
  NET_BOOT_MAX_RETRIES=$(vcgencmd bootloader_config | grep NET_BOOT_MAX_RETRIES)
  if [ -n "${NET_BOOT_MAX_RETRIES}" ]
  then
    echo "${NET_BOOT_MAX_RETRIES}"
    echo "--Specify the maximum number of times that the bootloader will"
  ...
  FREEZE_VERSION=$(vcgencmd bootloader_config | grep FREEZE_VERSION)
  if [ -n "${FREEZE_VERSION}" ]
  then
    echo "${FREEZE_VERSION}"
...
if [ -n "$(uname -a | grep -i "v8+\|PREEMPT\|aarch64")" ]
then
  BITS=64
else
  BITS=32
fi
echo "${PRETTY_NAME} (*** w/ ${BITS}-Bit Kernel ***)"
uname -a
echo
uptime -p
echo
...
echo "GPU: ${GPU} MB"
echo
echo "Note 1: GPU hardware-accelerated codecs will be disabled if \"gpu_mem=16\"."
...
then
  ...
  echo " GPU Temp: $(printf "%.2f" ${C})°C ($(printf "%.2f" ${F})°F) (***)"
else
  # Otherwise, show Centigrade/Celcius only
  echo " GPU Temp: $(printf "%2.f" ${C})°C"
fi
...
then
  # SUPPLEMENTAL CONVERSION to Farenheit
  F=$(echo "2 k 9 5 / ${C} * 32 + p" | dc)
  echo " ARM Temp: $(printf "%.2f" ${C})°C ($(printf "%.2f" ${F})°F) (***)"
else
  # Otherwise, show Centigrade/Celcius only
  echo " ARM Temp: $(printf "%.2f" ${C})°C"
fi
...
then
  ...
  then
    # SUPPLEMENTAL CONVERSION to Farenheit
    F=$(echo "2 k 9 5 / ${C} * 32 + p" | dc)
    echo "PMIC Temp: $(printf "%.2f" ${C})°C ($(printf "%.2f" ${F})°F) (***)"
  else
    # Otherwise, show Centigrade/Celcius only
    echo "PMIC Temp: $(printf "%.2f" ${C})°C"
  fi
fi
echo
...
#
# NOTE TO FUTURE SELF:
# tvservice has near zero use with vc4-kms-v3d(-pi4) as it will
# only report the last mode that the firmware requested.  Look
# at the /sys/class/drm/card1/(something)/state status dump.
