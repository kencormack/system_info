...
Letting the user specify a target file carries some risks. As such, the script will no longer allow invocation with sudo, as in "sudo system_info". Because someone, SOMEWHERE, will do something like specify the output as "-o /bin/login", or end up writing reports to directories they have no business putting reports in (like /bin. for example)... or wherever happened to be your current working directory at the time you ran it, and so on. "Forgotten" file locations are too easily blamed on the script "failing" to write the report. When the naming convention, and the location (the user's home directory) were a given, you always knew where you could later find the report. Personally, I saw little value in this option, versus just renaming and or moving the file to wherever you want it, after the report was run. But the task did make me clean up and consolidate the passing of command line parameters, so there is that.

If no filename is specified, a default is automatically set, similar to the legacy naming convention - I felt the legacy format had too many hyphens, so I removed a few. Now, instead of "yyyy-mm-dd-hh-mm-ss", you'll see "yyyymmdd-hhmmss" in the date/timestamp portion of the report filename.  Finally, if no path is specified with the filename, the script will attempt to write the report IN THE CURRENT DIRRECTORY (assuming you have write acess in that directory.  If you assign your own filename, and don't remember what directory you put it in, don't blame the script.  The summary at the bottom of the report will show you the full path of the file you created.

When a user specifies a target filename for the report, the script will enforce the following rules...
1.) No overwriting of an existing file. I can't be sure a user wouldn't clobber an important file they might otherwise have write access to.
2.) Making sure we have write access (as a non-root user) to the specified dir/file... user "pi" can't dump reports in "/lib/something-or-other"
3.) No running as root, which would negate the other restrictions (the account still needs sudo access, for several of the commands within the script that require root privileges to perform their tasks.)
4.) To eliminate risk of a user entering an asterisk, question mark, or other such nonsense, I limit valid characters in a filename to A-Z, a-z, 0-9, hyphen (-), underscore (_), period (.), and slash (/)"
