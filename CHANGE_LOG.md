...
CHANGES: V3.1.4
 1. Improved detection/testing of RTL-SDR dongles.
 2. Detection of Hauppauge USB TV Tuners added to Section 9.

CHANGES: V3.1.3
 1. Added missing check for supplemental package joystick.
 2. Added supplemental package evtest.
 3. Added input device 'supported events' reporting to Section 9.
 4. Added Kernel Compile-Time Configuration Settings to Section 3,
    showing which features were built as modules, which were
    compiled directly into the kernel, and which were left out.
 5. Added detection of supplemental package selinux-utils.
    The repository kernels still don't have support for SELINUX
    enabled, but it is possible to configure and recompile your
    own kernel with SELINUX enabled.

CHANGES: V3.1.2
 1. Added contents of /etc/xdg/lxsession/LXDE-pi/autostart
    and ${HOME}/.config/lxsession/LXDE-pi/autostart to the
    X-WINDOWS setion.
 2. Added GPU clock speed to "PROCESSOR SPEEDS" in Section 2.
 3. Added support for supplemental package pulseaudio
 4. Added pulseaudio details in Section 11
 5. Added test for aplay before aplay tests in Section 11
 6. Additional info added to alsa playback and capture devices
 7. Added detection of joysticks to Section 9.

CHANGES: V3.1.1
 1. Added "agnostics" as a required package.
 2. Added agnostics media speed test to PERFORMANCE section.
 3. Updated README.md to add agnostics as a required package,
    and to clarify, in the installation section, how to
    launch the appropriate version of the script based on
    whether your Pi is running buster, or the older stretch.
 4. Replaced all "if type -path" checks with a function, to
    confirm presence of a command.
 5. Added /dev/spidev* device nodes to Section 7

CHANGES: V3.1.0
 1. Tracked down a trio of uninitialized variables caught by
    "set -u", one of which only showed up in SSH, and not
    from a terminal window on the desktop.
 2. Enabled privileged mode with "set -p".  In this mode,
    the $BASH_ENV and $ENV files are not processed, shell
    functions are not inherited from the environment, and
    the SHELLOPTS, BASHOPTS, CDPATH and GLOBIGNORE variables,
    if they appear in the environment, are ignored.
 3. Enabled "set -C", preventing output redirection using
    ‘>’, ‘>&’, and ‘<>’ from overwriting existing files.
 4. Added additional instances of "|| :" to stop some non-0
    return codes from needlessly triggering the ERR trap.

CHANGES: V3.0.9
 1. Cleaned up link-local addresses to show cases where no
    link-local addresses are present.
 2. Added descriptions of some differences learned between
    "Raspbian" and "Raspberry Pi OS" to the README.md file,
    along with noting that the new Pico, being a micro-
    controller and not a full-fledged single-board-computer,
    is not supported by system_info.
 3. Added a few instances of "|| :" upon non-0 return codes
    by some innocuous commands, to avoid needlessly triggering
    the ERR trap.
 4. Enabled "set -u", to catch any uninitialized variables.
 5. Expanded the info in the display of the OS provenance,
    including date the SD image was generated, and the exact
    hash of the source code used to generate that SD image.
...
CHANGES: V3.0.8
 1. Added curl as a supplemental package.
 2. Added detection and name resolution of external ip
    address and name.
 3. List all link-local addresses.
 4. List all IPv4 addresses.
 5. List all IPv6 addresses.
 6. Added lookup of resolvable names.

CHANGES: V3.0.7
 1. Added [-v|--version] flag, updated usage and help screens.
 2. Moved detection of provenance (Raspbian or RPiOS) to
    fnCONFIRM_OS so that this info would be available in the
    event Option 3 is not selected.
 3. Do not display "(licensed)" for MPG2 and WVC1 codecs if
    OS is RPiOS and the model is a 4B or later (the licenses
    were only required/available for "Raspbian", and only
    for pre-4B models.)
 4. Under "Raspberry Pi OS", "dmesg | grep -i ft5406" no
    longer works to see if the Official 7" Touchscreen is
    detected.  Added an adition search pattern to accomodate
    RPiOS.
 5. Additional conversions of non-ascii box-drawing chars to
    ASCII (essentially, in uses of "systemctl status" and
    "systemd-analyze").
 6. Eliminated a useless "cat" command in a pipe to grep.

CHANGES: V3.0.6
 1. Convert non-ascii box-drawing chars to ASCII, in the
    output of certain commands, using iconv.
 2. Added "libc-bin" as a required package, for "iconv",
    which is used in change #1, above.
 3. Fixed a bug that occurs due to "shopt -s failglob".
    (I'm surprised no one reported this bug, which would
    cause the script to abort with an error, in section 8,
    on systems that lacked ttyACM? devices.)
 4. Detection of Raspbian/Raspberry Pi OS was reversed (Doh!)
 5. Fixed a bug in the display of which supplemental packages
    were found installed.  This is another bug I'm surprised
    no one reported.

CHANGES: V3.0.5
 1. Tweaks to SMART stuff, in the storage section.
 2. Changed a couple more "[" tests to bash's "[[" syntax.
 3. Distinguish between a legacy "Rasbian" and a
    "Raspberry Pi OS" installation, in menu option 3,
    OS Configuration.
 4. Added blocked/unblocked status of wireless devices to
    menu option 7, Hardware Busses.  Added "rfkill" to
    supplemental packages.
 5. Added PCI devices to option 7, Hardware Busses. Added
    pciutils to supplemental packages.

CHANGES: V3.0.4
 1. Added audit of "root" account password characteristics,
    home directory owner/group/perms, root's .ssh directory
    owner/group/perms, and group/owner/perms of key files
    in root's .ssh directory, to match the audits done for
    non-root, non-system acounts, in the section 16 security
    stuff.
 2. Fixed a bug with detection of lirc stuff.

CHANGES: V3.0.3
 1. Fixed a minor copy/paste error in the help screen.
 2. Added support for WiringPi v2.52 on the 8GB Pi 4B.
    I had assumed it would not support the rev [bcd]03114
    boards as it did the earlier revs, but after testing
    with my new 8GB unit, the 8GB is no longer excluded
    from the WiringPi test.  All 4Bs are treated the same.
 3. Updated the screenshots on the github page.
 4. Minor edits to README.md
 5. Added a listing of any supported supplemental packages
    that have been found, to the top of the report.

CHANGES: V3.0.2
 1. Added Pi 4B v1.4 2GB & 4GB rev codes, and modified the
    wiringpi exclusion check, accordingly.
 2. Added supplemental package "tmux".
 3. Rename fnCHECK_CMDLINE to fnOVERRIDE_RCFILE
 4. Rename fnMAIN_LOOP to fnMENU_LOOP
 5. Replaced a few occurrances of ~/filename with
    "${HOME}/filename"
 6. Disallow direct execution by root (or sudo).
    As a result of allowing for a user-specified report file
    (described below), direct execution by "root" is no longer
    allowed.  The script can only be run by a non-root userid.
    That userid however, still needs sudo privileges, as it
    always has.
 7. Updated the help screen
 8. Added a "Usage:" message showing valid syntax, if the user
    types an invalid parameter.
 8. Completely reworked the passing of commandline parameters.

This last change permits the following styles:
  POSIX-style: a single dash '-' followed by a single letter
  GNU-style - a double-dash '--' followed by a word
  A bastardization that allows for either a space between
  the option and it's parameter, or an equals sign '='
  between them.

Basically, the allowed options will be:
...
Letting the user specify a target file carries some risks. As such,
the script will no longer allow invocation with sudo, as in
"sudo system_info". Because someone, SOMEWHERE, will do something
like specify the output as "-o /bin/login", or end up writing
reports to directories they have no business putting reports in
(like /bin for example)... or wherever happened to be your current
working directory at the time you ran it, and so on. "Forgotten"
file locations are too easily blamed on the script "failing" to
write the report. When the naming convention, and the location
(the user's home directory) were a given, you always knew where you
could later find the report. Personally, I saw little value in this
option, versus just renaming and or moving the file to wherever you
want it, after the report was run. But the task did make me clean
up and consolidate the passing of command line parameters, so there
is that.

If no filename is specified, a default is automatically set, similar
to the legacy naming convention - I felt the legacy format had too
many hyphens, so I removed a few. Now, instead of "yyyy-mm-dd-hh-mm-ss",
you'll see "yyyymmdd-hhmmss" in the date/timestamp portion of the
report filename.  Finally, if no path is specified with the filename,
the script will attempt to write the report IN THE CURRENT DIRRECTORY
(assuming you have write access in that directory.  If you assign your
own filename, and don't remember what directory you put it in, don't
blame the script.  The summary at the bottom of the report will show
you the full path of the file you created.

When a user specifies a target filename for the report, the script
will enforce the following rules...
1.) No overwriting of an existing file. I can't be sure a user wouldn't
clobber an important file they might otherwise have write access to.
2.) Making sure we have write access (as a non-root user) to the
specified dir/file... user "pi" can't dump reports in
"/lib/something-or-other"
3.) No running as root, which would negate the other restrictions (the
account still needs sudo access, for several of the commands within the
script that require root privileges to perform their tasks.)
4.) To eliminate risk of a user entering an asterisk, question mark, or
other such nonsense, I limit valid characters in a filename to A-Z, a-z,
0-9, hyphen (-), underscore (_), period (.), and slash (/)"

CHANGES: V3.0.1
This update hardens the safeties built into the script, and
adds a profiling mechanism.

 1. Reworked handling of the lshw tmpfile, to use "mktemp",
    ensuring safe perms on the tmpfile.
 2. Added bash's "shopt -s failglob", so that globs that don't
    get expanded will cause errors, rather than getting passed
    to a command with the * intact.
 3. Removed "sudo" from the "readlink" used in determining "3>"
    debug data redirection.
 4. Using the same technique as the debugging log, along with
    PS4, this version adds a basic profiling log on fd4, for
    development use, separate from the other datastreams.

RENUMBERED VERSION TO 3.x, TO DENOTE END OF SUPPORT
