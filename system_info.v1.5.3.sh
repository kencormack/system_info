...
#   lshw, net-tools, rpi-eeprom, usbutils, sed, util-linux, v4l-utils,
#   wireless-tools
#
...
#   cups-client, dc, ethtool, lvm2, m4, mdadm, nfs-kernel-server, nmap,
#   perl-base, python3-gpiozero, quota, rng-tools, rpcbind, rtl-sdr, samba,
#   smartmontools, sysbench, sysstat, systemd-coredump, watchdog, wiringpi,
#   x11-xserver-utils
#
# NOTE:
...
then
  ...
  then
    ...
    SUPPLEMENTAL=$(dpkg -l 2>/dev/null | awk '{ print $2 }' | grep -i "^cups-client$\|^dc$\|^ethtool$\|^lvm2$\|^m4$\|^mdadm$\|^nfs-kernel-server$\|^nmap$\|^perl-base$\|^python3-gpiozero$\|^quota$\|^rng-tools$\|^rpcbind$\|^rtl-sdr$\|^samba$\|^smartmontools$\|^sysbench$\|^sysstat$\|^systemd-coredump$\|^watchdog$\|^wiringpi$\|^x11-xserver-utils$")
    SUP_HIT=0
    SUP_MAX=0
    for PACKAGE in cups-client dc ethtool lvm2 m4 mdadm nfs-kernel-server nmap perl-base python3-gpiozero quota rng-tools rpcbind rtl-sdr samba smartmontools sysbench sysstat systemd-coredump watchdog wiringpi x11-xserver-utils
    do
...
then
  fnBANNER " USB STORAGE WITH S.M.A.R.T. (***)"
  if [ -n "$(smartctl --scan -d ata | grep -v aborted)" ]
  then
    echo "Scanning ATA Devices..."
    smartctl --scan -d ata | grep -v aborted | awk '{ print $1 }' | while read DEV
    do
      fnSUB_BANNER " DEVICE: ${DEV}"
      ${SUDO} smartctl -d ata --xall ${DEV}
      echo
    done
  fi
  if [ -n "$(smartctl --scan -d scsi | grep -v aborted)" ]
  then
    echo "Scanning SCSI Devices..."
    smartctl --scan -d scsi | grep -v aborted | awk '{ print $1 }' | while read DEV
    do
      fnSUB_BANNER " DEVICE: ${DEV}"
      ${SUDO} smartctl -d scsi --xall ${DEV}
      echo
    done
  fi
fi
...
#---------------
fstrim -An >/dev/null 2>&1
FSTRIM_STATUS=${?}
case ${FSTRIM_STATUS} in
  0|64)
    fnBANNER " USB STORAGE WITH TRIM ENABLED"
    fstrim -Avn
    echo
    fnSUB_BANNER " UDEV RULES FOR TRIM"
    find /etc/udev -type f -exec grep unmap {} \; -print
    echo
    fnSUB_BANNER " FSTRIM.TIMER"
    systemctl status fstrim.timer
    echo
    if [ -n "$(grep discard /etc/fstab)" ]
    then
...
then
  ...
  do
    ...
    # See if this resolves a hang on first execute, on Pi 4 with usb boot beta firmware
    tvservice -a -v ${DISP_NUM} >/dev/null 2>&1
...
#
# Some people may not like the amount of writing this does to their
# SD card or SSD, so I include it here, but commented out.  If you
# want to run this next benchmark, simply uncomment the next few lines.
#
