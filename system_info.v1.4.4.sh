...
#   Pi 3B w/ Raspbian Buster (upgraded to Pi 4B w/2GB),
#   X150 9-port USB hub,
#   DVK512 board w/ RTC,
#   DockerPi Powerboard,
#   fit_StatUSB USB LED,
...
then
  ...
  then
    ...
    SUPPLEMENTAL=$(dpkg -l 2>/dev/null | awk '{ print $2 }' | grep -i "^cups-client$\|^dc$\|^ethtool$\|^nfs-kernel-server$\|^nmap$\|^python3-gpiozero$\|^rng-tools$\|^rpcbind$\|^rtl-sdr$\|^samba$\|^sysstat$\|^watchdog$\|^wiringpi$\|^x11-xserver-utils$")
    SUP_HIT=0
    SUP_MAX=0
    for PACKAGE in cups-client dc ethtool nfs-kernel-server nmap python3-gpiozero rng-tools rpcbind rtl-sdr samba sysstat watchdog wiringpi x11-xserver-utils
    do
...
#| grep -Ei '^[a-z0-9_]{6,32}$' \
#| sort -u \
#| xargs -i vcgencmd get_config {} \
#| grep = >/tmp/.config.${PPID} 2>/dev/null
...
then
  ...
  then
    ...
    cat /dev/hwrng | rngtest -c 1000 2>&1
    echo
  fi
fi
